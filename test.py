...
def concat_all_gather(tensors, num_total_examples):
    ...
    outputs = []
    for tensor in tensors:
        # 单 GPU 下无需 all_gather，直接处理
        tensor = tensor.cuda()  # 确保张量在 GPU 上（可选）
        output = tensor.cpu()   # 返回 CPU 张量（保持与原函数行为一致）
        if num_total_examples is not None:
            output = output[:num_total_examples]  # 可选：截断到指定长度
...
def test(config, model, attention, gap_classifier, gap_classifier_h, gap_classifier_b, queryloader, galleryloader,
    ...
    # # 创建文件处理器
    # file_handler = logging.FileHandler('reid_test.log')  # 日志文件路径
    ...
    # 测试日志
    # logger.info("This is a test log")  # 日志会写入 reid_test.log
