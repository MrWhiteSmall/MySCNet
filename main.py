...
world_size = 1  # Number of processes to run in parallel
rank = 0
...
def main(rank, world_size, config):
    ...
    for epoch in range(start_epoch, config.TRAIN.MAX_EPOCH):
        ...
        # if  True:
        if  (epoch + 1) % config.TEST.EVAL_STEP == 0 or (epoch + 1) == config.TRAIN.MAX_EPOCH:
            logger.info("==> Test")
            ...
            if rank == 0:
                ...
                save_checkpoint({
                    ...
                    'rank1': rank1,
                    'epoch': epoch,
                }, is_best, osp.join(config.OUTPUT, 'ckp_' + str(epoch + 1) + '.pth.tar'))
